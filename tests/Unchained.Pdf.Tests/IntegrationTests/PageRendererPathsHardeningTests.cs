...
public sealed class PageRendererPathsHardeningTests : RendererTestBase
{
    private static byte[] Page100(string content, string resources = "")
    {
        ...
        offsets.Add(Len());
        Ln("3 0 obj");
        Ln($"<< /Type /Page /Parent 2 0 R /MediaBox [0 0 100 100] /Contents 4 0 R /Resources << {resources} >> >>");
        Ln("endobj");
    ...
    private static byte[] SoftMaskTrianglePdf()
    {
        ...
        Ln("   /Resources << /ExtGState << /GS1 5 0 R >> >> >>");
        Ln("endobj");
        ...
        offsets.Add(Len());
        Ln("5 0 obj");
        Ln("<< /Type /ExtGState /SMask << /Type /Mask /S /Alpha /G 6 0 R >> >>");
        Ln("endobj");
        const string maskContent = "1 g 20 20 60 60 re f";
        var mc = Encoding.Latin1.GetBytes(maskContent);
        offsets.Add(Len());
        Ln("6 0 obj");
        Ln($"<< /Type /XObject /Subtype /Form /BBox [0 0 100 100] /Length {mc.Length} >>");
        sb.Append("stream\n").Append(maskContent);
        Ln("\nendstream");
        Ln("endobj");
        var xref = Len();
