...
public sealed class XmpMetadataTests : PdfTestBase
{
    ...
    {
        await using var doc = await LoadAsync(PdfFixtures.SinglePage(), TestContext.Current.CancellationToken);
        await Editor.SetXmpMetadataAsync(doc, SampleXmp, TestContext.Current.CancellationToken);
    ...
    public async Task SetThenRemoveMetadata_ReturnsNull()
    {
        await using var doc = await LoadAsync(PdfFixtures.SinglePage(), TestContext.Current.CancellationToken);
        await Editor.SetXmpMetadataAsync(doc, SampleXmp, TestContext.Current.CancellationToken);
        await Editor.RemoveMetadataAsync(doc, TestContext.Current.CancellationToken);
        doc.GetXmpMetadata().ShouldBeNull();
    }
