...
public sealed class PageRendererDispatchTests : RendererTestBase
{
    private static byte[] Page100(string content, string extraResources = "")
    {
        ...
        offsets.Add(Len());
        Ln("3 0 obj");
        Ln($"<< /Type /Page /Parent 2 0 R /MediaBox [0 0 100 100] /Contents 4 0 R /Resources << {extraResources} >> >>");
        Ln("endobj");
    ...
    private static byte[] UncolouredTilingPdf()
    {
        ...
        offsets.Add(Len());
        Ln("5 0 obj");
        Ln("<< /Type /Pattern /PatternType 1 /PaintType 2 /TilingType 1");
        Ln("   /BBox [0 0 10 10] /XStep 10 /YStep 10 /Resources << >>");
        Ln($"   /Length {cellBytes.Length} >>");
        sb.Append("stream\n").Append(cell);
        Ln("\nendstream");
        Ln("endobj");
        var xref = Len();
