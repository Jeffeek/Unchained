...
public sealed class RenderingPixelTests : RendererTestBase
{
    ...
    public async Task TextWithEmbeddedFont_MorePixelsThanBlankPage()
    {
        ...
        await using var blank = await LoadAsync(PdfFixtures.SinglePage(), ct: TestContext.Current.CancellationToken);
        var blankPng = await Renderer!.RenderPageAsync(blank.Pages[1], new RenderOptions(Dpi: 96), ct: TestContext.Current.CancellationToken);

        var fontData = LoadDejaVuBytes();
        var pdfBytes = PdfFixtures.WithEmbeddedFont(fontData,
            "BT /F1 14 Tf 50 700 Td (Hello, World!) Tj ET");
        await using var textDoc = await LoadAsync(pdfBytes, ct: TestContext.Current.CancellationToken);
        var textPng = await Renderer!.RenderPageAsync(textDoc.Pages[1], new RenderOptions(Dpi: 96), ct: TestContext.Current.CancellationToken);
    ...
    private static byte[] BuildPageWithFont(string baseFontName, string subtype, string contentStream)
    {
        ...
        byte[] L(string s) => System.Text.Encoding.Latin1.GetBytes(s + "\n");
