...
public sealed class TxtToPdfTests : PdfTestBase
{
    ...
    {
        await using var doc = await Processor.LoadFromTxtAsync("Hello, World!", cancellationToken: TestContext.Current.CancellationToken);
    ...
    {
        await using var doc = await Processor.LoadFromTxtAsync(string.Empty, cancellationToken: TestContext.Current.CancellationToken);
        doc.PageCount.ShouldBe(1);
    }
    ...
    public async Task LoadFromTxt_MultilineFitsOnOnePage()
    {
        ...
        await using var doc = await Processor.LoadFromTxtAsync(text, cancellationToken: TestContext.Current.CancellationToken);
        doc.PageCount.ShouldBe(1);
    }
    ...
    public async Task LoadFromTxt_LongLine_WordWrapsWithoutOverflow()
    {
        ...
        await using var doc = await Processor.LoadFromTxtAsync(longLine, cancellationToken: TestContext.Current.CancellationToken);
