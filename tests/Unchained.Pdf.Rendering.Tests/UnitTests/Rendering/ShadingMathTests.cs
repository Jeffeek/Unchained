...
namespace Unchained.Pdf.Rendering.Tests.UnitTests.Rendering;
...
public sealed class ShadingMathTests
{
    ...
    [Fact]
    public void TryInvertDeviceToUser_NonIdentity_RoundTrips()
    {
        // ctm = [2, 1, -1, 2, 10, 20], scale=1, pageHeightPt=100.
        // Forward: user (x,y) → ctm → (ux,uy) → device (ux*scale, (H-uy)*scale)
        // Composed: (2x - y + 10, -x - 2y + 80)
        double[] ctm = [2, 1, -1, 2, 10, 20];
        ShadingMath.TryInvertDeviceToUser(ctm, 1.0, 100, out var inv).ShouldBeTrue();
        inv.Length.ShouldBe(6);

        // User (5, 5) → device (2*5 - 5 + 10, -5 - 2*5 + 80) = (15, 65).
        // Inverse(15, 65) should give (5, 5).
        var (ux, uy) = ShadingMath.ApplyInv(inv, 15, 65);
        ux.ShouldBe(5, 0.001);
        uy.ShouldBe(5, 0.001);
    }
