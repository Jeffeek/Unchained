...
internal sealed class RasterBuffer(int width, int height)
{
    ...
    internal void SetClipRect(int x0, int y0, int x1, int y1)
    {
    ...
    internal void SetPixel(int x, int y, byte r, byte g, byte b, byte a, string blendMode = "Normal")
    {
        ...
        if (blendMode == "Normal" || blendMode == "Compatible")
        {
            if (a == 255)
            {
                _data[i] = r; _data[i + 1] = g; _data[i + 2] = b;
            }
            else
            {
                var inv = 255 - a;
        ...
        else
        {
            ...
            var br = _data[i]; var bg = _data[i + 1]; var bb = _data[i + 2];
            Blend(blendMode, br, bg, bb, r, g, b, a, out var or, out var og, out var ob);
            _data[i]     = or;
            _data[i + 1] = og;
            _data[i + 2] = ob;
        }
    ...
    private static void Blend(
        string mode,
        byte br, byte bg, byte bb,   // backdrop
        byte sr, byte sg, byte sb,   // source
        byte a,                      // source alpha
        out byte or, out byte og, out byte ob)
    {
        switch (mode)
        {
            ...
            case "Hue":
            {
                RgbToHsl(br, bg, bb, out _, out var sl, out var ll);
                RgbToHsl(sr, sg, sb, out var hs, out _, out _);
                HslToRgb(hs, sl, ll, out var mr, out var mg, out var mb);
                AlphaComposite(br, bg, bb, mr, mg, mb, a, out or, out og, out ob);
                break;
            }
            case "Saturation":
            {
                RgbToHsl(br, bg, bb, out var hb, out _, out var lb);
                RgbToHsl(sr, sg, sb, out _, out var ss, out _);
                HslToRgb(hb, ss, lb, out var mr, out var mg, out var mb);
                AlphaComposite(br, bg, bb, mr, mg, mb, a, out or, out og, out ob);
                break;
            }
            case "Color":
            {
                RgbToHsl(br, bg, bb, out _, out _, out var lb);
                RgbToHsl(sr, sg, sb, out var hs, out var ss, out _);
                HslToRgb(hs, ss, lb, out var mr, out var mg, out var mb);
                AlphaComposite(br, bg, bb, mr, mg, mb, a, out or, out og, out ob);
                break;
            }
            case "Luminosity":
            {
                RgbToHsl(br, bg, bb, out var hb, out var sb2, out _);
                RgbToHsl(sr, sg, sb, out _, out _, out var ls);
                HslToRgb(hb, sb2, ls, out var mr, out var mg, out var mb);
                AlphaComposite(br, bg, bb, mr, mg, mb, a, out or, out og, out ob);
                break;
            }
            default:
    ...
    [SuppressMessage("ReSharper", "BadListLineBreaks")]
    internal void FillRect(int x, int y, int w, int h, byte r, byte g, byte b, byte a = 255, string blendMode = "Normal")
    {
        var x1 = Math.Max(0, x);
        var y1 = Math.Max(0, y);
        ...
        for (var px = x1; px < x2; px++)
            SetPixel(px, py, r, g, b, a, blendMode);
    }
    ...
    internal void FillSpan(int y, int x0, int x1, byte r, byte g, byte b, byte a = 255, string blendMode = "Normal")
    {
        ...
        string blendMode = "Normal")
    {
        ...
        while (true)
        {
            ...
            for (var tx = -half; tx <= half; tx++)
                SetPixel(x0 + tx, y0 + ty, r, g, b, a, blendMode);
        ...
        string blendMode = "Normal")
    {
        for (var row = 0; row < glyphHeight; row++)
        {
            ...
            for (var col = 0; col < glyphWidth; col++)
            {
                ...
                SetPixel(destX + col, destY + row, r, g, b, alpha, blendMode);
            }
        }
    }
    ...
    internal void BlendPixel(int x, int y, byte r, byte g, byte b, byte a, string blendMode = "Normal") =>
        SetPixel(x, y, r, g, b, a, blendMode);
