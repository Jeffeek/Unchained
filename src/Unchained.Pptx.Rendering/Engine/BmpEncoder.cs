...
internal static class BmpEncoder
{
    ...
    internal static byte[] Encode(RasterBuffer buffer)
    {
        ...
        var rowStride = ((width * 3) + 3) & ~3; // pad each row to 4 bytes
        ...
        BinaryPrimitives.WriteInt32LittleEndian(info[8..], height); // positive = bottom-up
        BinaryPrimitives.WriteInt16LittleEndian(info[12..], 1);     // colour planes
