...
public static class AgileEncryption
{
    ...
    /// </summary>
    /// <exception cref="OoXmlEncryptedException">
    ...
    public static byte[] Decrypt(byte[] cfbBytes, string password)
    {
        ...
        if (!streams.TryGetValue("EncryptionInfo", out var encryptionInfoStream))
            throw new OoXmlEncryptedException("EncryptionInfo stream not found.");

        if (!streams.TryGetValue("EncryptedPackage", out var encryptedPackage))
            throw new OoXmlEncryptedException("EncryptedPackage stream not found.");

        if (encryptionInfoStream.Length < 8)
            throw new OoXmlEncryptedException("EncryptionInfo stream too short.");
        ...
        {
            throw new OoXmlEncryptedException(
    ...
    private static byte[] DecryptPackage(byte[] encryptedPackage, byte[] documentKey, byte[] salt)
    {
        if (encryptedPackage.Length < 8)
            throw new OoXmlEncryptedException("EncryptedPackage too short.");
        ...
        ParseEncryptionInfo(ReadOnlySpan<byte> xmlSpan)
    {
        ...
        var encKeyEl = doc.Root?.Descendants(pwd + "encryptedKey").FirstOrDefault()
                       ?? throw new OoXmlEncryptedException("Missing encryptedKey element.");
        ...
        static byte[] B64(XElement el, string attr) =>
            Convert.FromBase64String(
                ...
                ?? throw new OoXmlEncryptedException($"Missing attribute: {attr}")
            );
    }
