...
namespace Unchained.Pptx.Core.Opc;
...
internal sealed class OpcPackage : IDisposable
{
    ...
    /// <exception cref="PptxException">Thrown when the bytes do not form a valid OPC package.</exception>
    ...
    /// <exception cref="PptxException">Thrown when the stream does not contain a valid OPC package.</exception>
    public static OpcPackage Open(Stream stream)
    {
        ...
        {
            throw new PptxException("The stream does not contain a valid PPTX / OPC package.", ex);
        }
    }
    ...
    /// <exception cref="PptxException">Thrown when no part with that URI exists.</exception>
    public OpcPart GetPart(string partUri)
    {
        var part = TryGetPart(partUri)
            ?? throw new PptxException($"OPC part not found: '{partUri}'.");
        return part;
    }
    ...
    /// <summary>
    /// Serializes the package to a byte array in PPTX (ZIP) format.
    /// </summary>
    public byte[] Save()
    {
    ...
    public void SaveTo(Stream stream)
    {
        ...
        // Write each part and its relationships
        foreach (var part in _parts.Values)
        {
            // Strip leading slash for ZIP entry names
    ...
    {
        // First pass — collect raw bytes keyed by entry name
        ...
        foreach (var (name, data) in entries)
        {
            ...
            // Parse part-level relationships if present
