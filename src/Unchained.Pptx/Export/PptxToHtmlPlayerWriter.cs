...
internal static class PptxToHtmlPlayerWriter
{
    ...
    private static void WriteStyle(StringBuilder sb, double slideW, double slideH, HtmlPlayerSaveOptions options)
    {
        sb.AppendLine("<style>");
        ...
        sb.AppendLine($".slide{{position:relative;width:{slideW:F2}px;height:{slideH:F2}px;overflow:hidden;background:white;transform-origin:center center}}");
        ...
        sb.AppendLine("#chrome{position:fixed;bottom:0;left:0;right:0;display:flex;gap:12px;align-items:center;justify-content:center;padding:8px;background:rgba(0,0,0,.55);color:#eee;font-size:14px;user-select:none}");
        sb.AppendLine("#chrome button{background:#333;color:#eee;border:1px solid #555;border-radius:4px;padding:4px 10px;cursor:pointer;font-size:14px}");
    ...
    private static void WriteScript(StringBuilder sb, int count)
    {
        ...
        sb.AppendLine("function show(i){if(i<0)i=0;if(i>total-1)i=total-1;pages[cur].style.display='none';pages[i].style.display='block';cur=i;if(counter)counter.textContent=(i+1)+' / '+total;scale();}");
        sb.AppendLine("function scale(){var p=pages[cur];var s=p.querySelector('.slide');if(!s)return;var sw=s.offsetWidth,sh=s.offsetHeight;var k=Math.min(window.innerWidth/sw,(window.innerHeight-48)/sh);s.style.transform='scale('+k+')';}");
        ...
        sb.AppendLine("document.getElementById('fs').addEventListener('click',function(){if(!document.fullscreenElement){document.documentElement.requestFullscreen&&document.documentElement.requestFullscreen();}else{document.exitFullscreen&&document.exitFullscreen();}});");
        sb.AppendLine("document.addEventListener('keydown',function(e){switch(e.key){case'ArrowRight':case' ':case'PageDown':next();e.preventDefault();break;case'ArrowLeft':case'PageUp':prev();e.preventDefault();break;case'Home':show(0);break;case'End':show(total-1);break;}});");
