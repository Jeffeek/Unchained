...
public sealed class SpreadsheetDocument : ISpreadsheetDocument
{
    ...
    internal SharedStringsTable SharedStrings
    {
        get
        {
            ...
            var part = Package?.Parts.FirstOrDefault(static p => p.ContentType.Equals(SmlNames.ContentTypeSharedStrings, StringComparison.Ordinal));
    ...
    public StyleBook Styles
    {
        get
        {
            ...
            var part = Package?.Parts.FirstOrDefault(static p =>
                p.ContentType.Equals(SmlNames.ContentTypeStyles, StringComparison.Ordinal)
            );
            MaterialisedStyles = part != null
                ? StylesParser.Parse(part.Data)
    ...
    /// </summary>
    public int Recalculate() => FormulaCalculator.Recalculate(this);
    ...
    public static object? EvaluateFormula(Worksheet contextSheet, string formula)
    {
        ...
        var evaluator = new FormulaEvaluator(contextSheet);
        var result = evaluator.Evaluate(formula.StartsWith('=') ? formula[1..] : formula);
        return result.Kind switch
        {
            FormulaValueKind.Number => result.Number,
            FormulaValueKind.Boolean => result.Boolean,
            FormulaValueKind.Text => result.Text,
            FormulaValueKind.Error => result.Error,
            _ => null
        };
    }
