...
internal sealed class PdfFunction
{
    ...
    private PdfFunction(int functionType, double[] domain, double[] c0, double[] c1, double n,
    ...
    internal static PdfFunction? Build(PdfObject? obj, PdfDocumentCore core, int depth = 0)
    {
        ...
        switch (ft)
        {
            case 2:
            {
                ...
                return new PdfFunction(2, domain, c0, c1, n, [], [], []);
            }
            case 3:
            {
                ...
                return fns.Length == 0 ? null : new PdfFunction(3, domain, [], [], 1, fns, bounds, encode);
            }
            default:
                // Types 0 / 4: approximate with the dict's C0/C1 if present, else null.
                {
                    ...
                    if (c0 is not null && c1 is not null)
                        return new PdfFunction(2, domain, c0, c1, 1, [], [], []);
                    return null;
                }
        }
    }
